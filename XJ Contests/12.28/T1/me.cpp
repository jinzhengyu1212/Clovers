...
namespace solver2{
    ...
    void main(){
        ...
        for(int i=1;i<m;i++){
            for(int j=i+1;j<=m;j++){
                int lca2=L[j],lca1=L[i];
                //int ttt=0; if(i==3&&j==5) ttt=1;
                ...
                if(isfa(lca1,lca2)){
                    ...
                    if(!isfa(lca2,x2)) continue;
                    if(dis+dep[lca2]-dep[lca1]==dep[y1]-dep[lca2]){
                        ans++; 
                        //cout<<i<<" "<<j<<endl;
                        continue;
                    }
                    //if(ttt) outln(dis),out(x1),out(x2),out(lca1),out(y1),out(y2),outln(lca2);
                    if(isfa(lca2,x2)){
                        int tmp=LCA(x2,y1);
                ...
                else if(isfa(lca2,lca1)&&isfa(lca1,y1)){
