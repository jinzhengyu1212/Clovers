...
int main() {
	...
	rep(q) {
		...
		if(c)
			...
			if(uf.unite(a, b)) components--;
		}
	}

	if(v.empty()) {
		Yes(in_range(m, n - 1, components * (components - 1) / 2 + n - components + 1));
	}
	...
	Yes(in_range(m, n, components * (components - 1) / 2 + n - components + 1));
